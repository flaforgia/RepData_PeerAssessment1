...
```{r fig1_mean_step, fig.height=5, fig.width=10, fig.path='figure/'}
...
```{r fig2_mean_increment, fig.height=5, fig.width=10, fig.path='figure/'}
...
So the max average value of the number of steps is **`r MaxSteps`** and the increment of time at which this maximum occurs is **`r MaxInc`**.
...
The Missing value represents **`r ValueNA`** observation which correspond to 8 full days.  
...
```{r fig3_missing_values, fig.height=5, fig.width=10, fig.path='figure/'}
colors <- c("red", "yellow", "green", "violet", "orange", "blue", "pink", "cyan") 
...
- Previous Values    
    * Mean: `r round(MeanPerDayNoNA, digits=2)` (`r MeanPerDayNoNA`)  
    * Median: `r MedianPerDayNoNA`  
- New Values     
    * Mean: `r round(MeanPerDayNA, digits=2)` (`r MeanPerDayNA`)  
    * Median: `r MedianPerDayNA`    
...
```{r fig4_wekdays_weekends, fig.height=9, fig.width=12, fig.path='figure/'}
par(mfrow = c(2,1))
...
The plot shows that there is a relation between the days of the week and the activity. The activity is more important during the early morning in the weekdays. This seems to show that the user is someone who do sport in the morning because the maximum steps is in the morning (`r round(MaxSteps, digits=2)`, `r MaxInc`). This maximum is reached only during the weekdays as the cross comparison between the plots can show. Also the peaks during the working hours are see are located in the weekend plot.  
